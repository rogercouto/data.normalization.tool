...
/**
 * Utility class used for clusterize column words 
 */
public class Cluster {

	private int editDistance;
	private HashMap<String, List<WordCounter>> map = new HashMap<>();
	...
	public void add(String value){
		if (map.containsKey(value)){
			...
			if (!found){
				...
				c.setValue(value);
				c.setCount(1);
				List<WordCounter> l = new ArrayList<>();
				l.add(c);
				map.put(value, l);
			}
		}
	...
	private static int getIndex(String value, List<WordCounter> list){
		int index = -1;
		int count = 0;
		for (WordCounter counter : list) {
	...
	public String getHigher(String value){
		if (map.containsKey(value)){
			...
			if (oi.isPresent()){
				Optional<WordCounter> oc = map.get(value).stream().filter(c->c.getCount() == oi.getAsInt()).findFirst();
				if (oc.isPresent())
			...
			for (String key : map.keySet()) {
				...
				if (same){
					...
					if (oi.isPresent()){
						Optional<WordCounter> oc = map.get(key).stream().filter(c->c.getCount() == oi.getAsInt()).findFirst();
						if (oc.isPresent())
							return oc.get().getValue();
					}
				}
	...
	public HashMap<String, List<WordCounter>> getMap() {
		return map;
	}
